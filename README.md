...
Current release: **Beta-0.0.66**. During beta development, each feature build
increments the patch number (`Beta-0.0.66`, `Beta-0.0.66`, and so on).
...
Open `js/twitch-config-v66.js` and paste it in:
...
`redirectUriOverride` in `js/twitch-config-v66.js` to the one production callback you
...
js/
  twitch-config-v66.js # Client ID, scopes, redirect URI
  ...
  wormhole-app-v66.js # Wires everything together, renders the UI
tests/
