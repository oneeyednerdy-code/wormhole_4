...
test('filters module imports viewer tolerance helpers used after modularization', () => {
  assert.match(source, /import\s*\{[^}]*describeViewerRange[^}]*parseViewerTolerance[^}]*\}\s*from\s*['"]\.\.\/viewer-tolerance\.js\?v=91['"]/s);
});
...
test('filters module imports all helpers used by interactive tag and genre controls', () => {
  assert.match(source, /import \{ normalizeTagKey \} from '\.\.\/tag-display\.js\?v=91';/);
  assert.match(source, /import \{ logger \} from '\.\.\/app\/logger\.js\?v=91';/);
  assert.match(source, /import \{ startLoading, finishLoading \} from '\.\.\/loading-state\.js\?v=91';/);
