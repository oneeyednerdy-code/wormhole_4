...
import { TwitchAuth } from './twitch-auth.js?v=73';
import { TwitchApi } from './twitch-api.js?v=73';
import { applyHardFilters, findRaidMatches } from './raid-match.js?v=73';
import { RaidListener } from './raid-listener.js?v=73';
import { ViewerHistory } from './viewer-history.js?v=73';
import { PreviousStreamHistory } from './previous-stream-history.js?v=73';
import { paginate } from './pagination.js?v=73';
import { sortRaidMatches } from './result-sort.js?v=73';
import { calculateViewerRange, parseViewerTolerance } from './viewer-tolerance.js?v=73';
import {
  createRaidCountdown,
  getRaidCountdownSnapshot,
} from './raid-countdown.js?v=73';
import { ChannelHistory } from './channel-history.js?v=73';
import { estimateStreamEnd, parseTwitchDuration } from './stream-end-estimate.js?v=73';
import {
  getGenreGameNames,
  getGenreLabelsForGame,
} from './genre-presets.js?v=73';
import { isLanguageTag, parseTagInput } from './language-tags.js?v=73';
import { getSearchedTagMatch, normalizeTagKey, prepareTagDisplay } from './tag-display.js?v=73';
import { normalizeTwitchLogin } from './direct-search.js?v=73';
import { buildTwitchPlayerUrl, buildTwitchWatchUrl } from './twitch-player.js?v=73';
import { buildFollowedDirectoryMatches } from './followed-directory.js?v=73';
import { resolveDiscoveryMode } from './discovery-mode.js?v=73';
import { loadFilterPreset, saveFilterPreset } from './filter-preset-storage.js?v=73';
import {
  getRaidDestinationEmbedUrls,
  getTwitchRaidControlsUrl,
  isMatchingRaidConfirmation,
} from './raid-completion.js?v=73';
import { DiagnosticsLog } from './diagnostics.js?v=73';
...
} from './content-label-filter.js?v=73';
...
const el = {
  ...
  matchPresetHint: document.getElementById('match-preset-hint'),
...
async function attachContentClassificationLabels(streams, { signal } = {}) {
  ...
  try {
    ...
    console.warn('Content classification labels are temporarily unavailable:', error);
  }
  return streams;
}
...
el.loginBtn.addEventListener('click', () => {
  if (TWITCH_CONFIG.clientId === 'YOUR_TWITCH_CLIENT_ID') {
    el.loginError.textContent =
      'Set your Twitch Client ID in js/twitch-config-v73.js before logging in.';
    return;
  }
  try {
...
async function init() {
  ...
  state.api = new TwitchApi(token, {
    onError: (event) => diagnostics.record({
      ...
      details: { endpoint: event.endpoint, method: event.method, status: event.status },
    }),
  });
...
const MATCH_PRESET_DESCRIPTIONS = {
  similar: 'Prioritizes channels with viewer counts, average audiences, stream lengths, categories, and tags similar to yours.',
  growth: 'Prioritizes a step-up raid, aiming near 150% of your current live audience while still respecting your other filters.',
  familiar: 'Prioritizes channels you already follow and meaningful shared tags. A shared Twitch team helps when team matching is enabled.',
  explore: 'Prioritizes channels you do not follow and, when your selected categories allow it, categories different from your primary one.',
};

function renderMatchPresetHint() {
  el.matchPresetHint.textContent = MATCH_PRESET_DESCRIPTIONS[el.matchPreset.value]
    ?? MATCH_PRESET_DESCRIPTIONS.similar;
}
...
function getContentLabelFilter() {
  const include = [];
  const exclude = [];
...
function applyContentLabelFilterToControls(value) {
...
async function runSearch() {
  ...
  try {
    ...
    if (
      ...
      showToast('Some channels were omitted because Twitch content labels were unavailable.', true);
    ...
    if (wantsOpenChatOnly) {
      ...
      if (chatModeCandidates.length) {
        showSearchStatus(`Checking chat access for ${chatModeCandidates.length} compatible channel${chatModeCandidates.length === 1 ? '' : 's'}…`);
        ...
        if (availableCount < chatModeCandidates.length && !state.chatSettingsWarningShown) {
          showToast('Some channel chat settings were unavailable and were excluded.', true);
...
function matchReasons(match) {
  ...
  const reasons = match.goalMatchReason ? [match.goalMatchReason] : [];
...
function renderResults() {
  ...
  loadFollowerDetailsForVisiblePage(page.items, renderGeneration);
...
async function loadFollowerDetailsForVisiblePage(matches, renderGeneration) {
  ...
  el.resultsList.querySelectorAll('[data-follows-you-id]').forEach((node) => {
    ...
    const follows = followsYou.get(id);
  ...
  el.resultsList.querySelectorAll('[data-chat-settings-id]').forEach((node) => {
    const id = node.dataset.chatSettingsId;
    const settings = chatSettings.get(id) ?? null;
    const match = matches.find((candidate) => candidate.stream.user_id === id);
    if (match) match.stream.chat_settings = settings;
...
el.matchPreset.addEventListener('change', () => {
  renderMatchPresetHint();
  onFilterChanged();
});
  ...
  return `
    <li class="result-card">
      ...
      <div class="stat-row">
        ...
        <span class="stat-chip" data-follows-you-id="${escapeHtml(s.user_id)}">Checking follow-back…</span>
