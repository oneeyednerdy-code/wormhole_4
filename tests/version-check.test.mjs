...
test('an old page clears outdated Wormhole caches and reloads with the latest version', async () => {
  ...
  const result = await ensureCurrentRelease({
    ...
    fetchImpl: async () => manifestResponse('0.0.65', '65'),
  });
  ...
  assert.equal(next.searchParams.get('wormhole_version'), '0.0.65');
...
test('cache cleanup never removes unrelated or current caches', async () => {
  ...
  assert.deepEqual(cleared, ['wormhole-59', 'wormhole-60', 'wormhole-61', 'wormhole-62', 'wormhole-63', 'wormhole-64']);
  assert.deepEqual(deleted, ['wormhole-59', 'wormhole-60', 'wormhole-61', 'wormhole-62', 'wormhole-63', 'wormhole-64']);
});
