import { TWITCH_CONFIG } from './config.js?v=41';
import { TwitchAuth } from './twitch-auth.js?v=41';
import { TwitchApi } from './twitch-api.js?v=41';
import { applyHardFilters, findRaidMatches } from './raid-match.js?v=41';
import { RaidListener } from './raid-listener.js?v=41';
import { ViewerHistory } from './viewer-history.js?v=41';
import { PreviousStreamHistory } from './previous-stream-history.js?v=41';
import { paginate } from './pagination.js?v=41';
import { sortRaidMatches } from './result-sort.js?v=41';
import { calculateViewerRange, parseViewerTolerance } from './viewer-tolerance.js?v=41';
import {
  createRaidCountdown,
  getRaidCountdownSnapshot,
} from './raid-countdown.js?v=41';
import { ChannelHistory } from './channel-history.js?v=41';
import { estimateStreamEnd, parseTwitchDuration } from './stream-end-estimate.js?v=41';
import {
  getGenreGameNames,
  getGenreLabelsForGame,
} from './genre-presets.js?v=41';
import { applyLanguageTag, isLanguageTag } from './language-tags.js?v=41';
import { prepareTagDisplay } from './tag-display.js?v=41';
import { normalizeTwitchLogin } from './direct-search.js?v=41';
import { buildFollowedDirectoryMatches } from './followed-directory.js?v=41';
import {
  getRaidDestinationEmbedUrls,
  getTwitchRaidControlsUrl,
  isMatchingRaidConfirmation,
  RAID_COMPLETION_MESSAGE,
} from './raid-completion.js?v=41';
...
function showRaidAwaitingConfirmation() {
  ...
  el.raidProgressText.textContent = 'The countdown ended. No chat message will be sent until Twitch confirms that the raid appeared in chat.';
...
async function handleRaidCompleted(event) {
  ...
  el.raidProgressText.textContent = `Twitch confirmed the raid to ${target.userName}. Sending the completion message…`;
  el.raidProgressCancelBtn.disabled = true;
  ...
  showRaidDestination(target, { delivery, deliveryError });
}

function showRaidDestination(target, { delivery, deliveryError } = {}) {
  const embeds = getRaidDestinationEmbedUrls(target.userLogin, window.location.hostname);
  const messageStatus = deliveryError
    ? 'The raid completed, but Twitch could not send the Wormhole completion message.'
    : delivery?.is_sent
      ? `Raid complete. “${RAID_COMPLETION_MESSAGE}” was sent to chat.`
