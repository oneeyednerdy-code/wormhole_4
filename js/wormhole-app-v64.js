import { TWITCH_CONFIG } from './twitch-config-v64.js?v=64';
import { TwitchAuth } from './twitch-auth.js?v=64';
import { TwitchApi } from './twitch-api.js?v=64';
import { applyHardFilters, findRaidMatches } from './raid-match.js?v=64';
import { RaidListener } from './raid-listener.js?v=64';
import { ViewerHistory } from './viewer-history.js?v=64';
import { PreviousStreamHistory } from './previous-stream-history.js?v=64';
import { paginate } from './pagination.js?v=64';
import { sortRaidMatches } from './result-sort.js?v=64';
import { calculateViewerRange, parseViewerTolerance } from './viewer-tolerance.js?v=64';
import {
  createRaidCountdown,
  getRaidCountdownSnapshot,
} from './raid-countdown.js?v=64';
import { ChannelHistory } from './channel-history.js?v=64';
import { estimateStreamEnd, parseTwitchDuration } from './stream-end-estimate.js?v=64';
import {
  getGenreGameNames,
  getGenreLabelsForGame,
} from './genre-presets.js?v=64';
import { applyLanguageTag, isLanguageTag } from './language-tags.js?v=64';
import { prepareTagDisplay } from './tag-display.js?v=64';
import { normalizeTwitchLogin } from './direct-search.js?v=64';
import { buildTwitchPlayerUrl, buildTwitchWatchUrl } from './twitch-player.js?v=64';
import { buildFollowedDirectoryMatches } from './followed-directory.js?v=64';
import { resolveDiscoveryMode } from './discovery-mode.js?v=64';
import { loadFilterPreset, saveFilterPreset } from './filter-preset-storage.js?v=64';
import {
  buildRaidCompletionMessage,
  getRaidDestinationEmbedUrls,
  getTwitchRaidControlsUrl,
  isMatchingRaidConfirmation,
} from './raid-completion.js?v=64';
...
el.loginBtn.addEventListener('click', () => {
  if (TWITCH_CONFIG.clientId === 'YOUR_TWITCH_CLIENT_ID') {
    el.loginError.textContent =
      'Set your Twitch Client ID in js/twitch-config-v64.js before logging in.';
    return;
  }
  try {
...
async function runSearch() {
  ...
  try {
    const {
      ...
      primaryGameId: state.myStream?.game_id,
...
function streamTagsHtml(match) {
  const searchedTags = state.resultsMode === 'matches' ? getTagsQuery() : [];
...
function renderTagMatchLegend() {
  const showSearched = state.resultsMode === 'matches' && getTagsQuery().length > 0;
...
async function loadFollowerDetailsForVisiblePage(matches, renderGeneration) {
  ...
  const [counts, followsYou] = await Promise.all([
...
function resultCardHtml(match, rank) {
  ...
  const followingText = s.followed_at
    ...
    : 'Following';
  const twitchChannelUrl = buildTwitchWatchUrl({ channel: s.user_login });
  const followAction = !s.is_followed && twitchChannelUrl
    ? `<a class="btn btn--ghost" href="${escapeHtml(twitchChannelUrl)}" target="_blank" rel="noopener noreferrer" title="Opens Twitch where you can follow this channel" aria-label="Follow ${escapeHtml(s.user_name)} on Twitch">Follow on Twitch ↗</a>`
    : '';
